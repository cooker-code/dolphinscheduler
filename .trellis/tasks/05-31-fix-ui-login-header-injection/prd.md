...
- [ ] 刷新 `http://localhost:5173/login` 后，控制台不再新增 `TypeError: config.headers.set is not a function`。
- [ ] 点击“登录”时，登录请求不会被请求拦截器的 headers 写入逻辑中断。
- [ ] `sessionId` 和 `language` header 的注入语义保持不变。
- [ ] `pnpm exec eslint src/service/service.ts` 通过。
- [ ] `pnpm exec prettier --check src/service/service.ts` 通过。
