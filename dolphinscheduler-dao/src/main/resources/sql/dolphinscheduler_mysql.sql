...
(
    ...
    `datasource_type`       VARCHAR(64)    DEFAULT NULL COMMENT 'datasource type, e.g. HIVE, MYSQL',
    `sql_content`           MEDIUMTEXT     DEFAULT NULL COMMENT 'SQL content (editor draft)',
    `workflow_code`         BIGINT         DEFAULT NULL COMMENT 'linked workflow definition code after deploy',
    `user_id`               INT            DEFAULT NULL COMMENT 'creator user id',
    `create_time`           DATETIME       NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'create time',
    `update_time`           DATETIME       NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'update time',
    PRIMARY KEY (`id`),
